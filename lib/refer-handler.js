...
class ReferHandler {
  constructor(opts) {
    ...
    assert(typeof opts.transfereeDialog === 'object',
      '\'transfereeDialog\' is a required option for refer handling and must be an object of the Dialog class');
    assert(typeof opts.transferorDialog === 'object',
    ...
    this.transfereeDialog = opts.transfereeDialog;
    this.transferorDialog = opts.transferorDialog;
    if (opts.auth) {
      this.auth = opts.auth;
    }
    if (opts.destinationLookUp) {
  ...
  referHandler() {
    return new Promise(async(resolve, reject) => {
      ...
      const referParams = this.referToUriParser(referTo.uri);

      referParams.referredBy = this.req.getParsedHeader('Referred-By'); // need to parse for the to user

      if (this.auth) {
        if (!this.authLookup(referParams)) {
          this.res.send(403);
          return;
        }
      }
      ...
      this.transferorDialog.on('destroy', () => {
        debug('ReferHandler# transferor dialog destroyed.');
      });
      ...
      try {
        if (referParams.replaces) {
          ...
          [transferTargetDialog, this.transfereeDialog].forEach((dialog) => {
            dialog.on('destroy', () => {
              debug('ReferHandler#_handleReplacesRefer call ended');
        ...
        reject(`ReferHandler# error transfering call to ${referParams.user}`);
      }
    });
  }
}
